...
# Chat input
# --- REPLACE THE OLD CHAT BLOCK WITH THIS ---
if prompt := st.chat_input("What would you like to know?"):
    st.session_state.messages.append({"role": "user", "content": prompt})
    with st.chat_message("user"):
        st.markdown(prompt)

    # Use the knowledge base ID from the Stack 2 terminal output
    # You will need to paste this in the sidebar of the app

    if valid_prompt(prompt, model_id):
        # Query Knowledge Base - THIS HAS CHANGED
        context, sources = query_knowledge_base(prompt, kb_id)

        # Generate response using LLM
        full_prompt = f"Context: {context}\n\nUser: {prompt}\n\n"
        response = generate_response(full_prompt, model_id, temperature, top_p)

        # --- NEW CODE FOR "STAND OUT" ---
        ...
        response = "I'm unable to answer this, please try again. This chatbot only answers questions related to heavy machinery."
